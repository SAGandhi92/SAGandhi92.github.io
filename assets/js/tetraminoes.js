...
Block.prototype.wallcollide = function(oldsquares, dir){
		...
		if(len==0){
			...
			case 'left' : for (var i=0; i<4; i++){
				for (var j=0; j<len; j++){
					if(this.squares[i].x +md > this.game.world.bounds.width || (this.squares[i].x < oldsquares[j].x && this.squares[i].x +3*md> oldsquares[j].x && this.square[i].y == oldsquares[j].y))
					return true;
				}
			}
			break;
